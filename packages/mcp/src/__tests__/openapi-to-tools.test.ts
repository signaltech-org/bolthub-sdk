...
describe("buildToolName", () => {
  test("builds a source-local name from method and path (the aggregator adds the key prefix)", () => {
    expect(buildToolName("get", "/v2/pokemon")).toBe("get_v2_pokemon");
  });
});
...
describe("convertOpenApiToTools", () => {
  test("converts full spec with multiple paths", () => {
    ...
    expect(tools[0].name).toBe("get_v2_pokemon");
    ...
    expect(tools[2].name).toBe("post_v2_abilities");
