...
/** @internal */
export function buildToolName(slug: string, method: string, path: string): string {
  const normalized = path.replace(/\//g, "_").replace(/[^a-zA-Z0-9_]/g, "");
  return `${slug}_${method.toLowerCase()}${normalized}`;
}
...
export function convertOpenApiToTools(
  ...
  const slug = extractSlug(gatewayUrl);
  ...
  for (const [path, methods] of Object.entries(spec.paths)) {
    for (const [method, operation] of Object.entries(methods)) {
      ...
      tools.push({
        name: buildToolName(slug, method, path),
